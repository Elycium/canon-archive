...
export function readGrokProjectId() {
  return String((typeof process !== "undefined" ? process.env?.VITE_PROJECT_ID : "") ?? "").trim();
}
...
export function grokOgHeadTags({ host = "", appName = DEFAULT_APP_NAME, site = {}, documentTitle = "" } = {}) {
...
export function normalizeHeadContext(ctx = {}) {
  ...
  return { appName, projectId: ctx.projectId ?? readGrokProjectId(), creator: ctx.creator ?? readXCreator(), creatorId: ctx.creatorId ?? readXCreatorId(), host: ctx.host ?? "", cwd, site };
}
...
export function createHeadInjector(ctx = {}) {
  ...
  let pending = [];
  let done = false;
  const apply = (html) => injectGrokPwaHead(html, normalized);
  return {
    push(chunk) {
